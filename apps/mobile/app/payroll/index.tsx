...
const sampleAdvances: SalaryAdvance[] = [
  ...
  },
  {
    ...
    approvedAmount: 500,
    status: "completed",
    requestDate: new Date(Date.now() - 2592000000).toISOString(),
    repaidAmount: 500,
  },
  {
    id: "sa3",
...
export default function PayrollModule() {
  ...
  const activeAdvances = advances.filter(
    (a) => a.status === "repaying" || a.status === "pending",
  ).length;
              ...
              <View key={a.id} style={styles.advCard}>
                ...
                {a.status === "repaying" && (
                  <View style={styles.progressBar}>
                    <View
                      style={[
                        styles.progressFill,
                        { width: `${(a.repaidAmount / a.approvedAmount) * 100}%` },
                      ]}
                    />
                  </View>
                )}
                ...
                {a.status === "pending" && (
                  <View style={styles.advActions}>
                    <Pressable
                      onPress={() => approveAdvance(a.id)}
                      style={[styles.advBtn, { backgroundColor: Colors.accent.green }]}
                    >
                      <Text style={styles.advBtnText}>Approve</Text>
                    </Pressable>
                    <Pressable
                      onPress={() => rejectAdvance(a.id)}
                      style={[styles.advBtn, { backgroundColor: Colors.accent.red }]}
                    >
          ...
          <View style={styles.section}>
            ...
            <View style={styles.uploadCard}>
              ...
              <Text style={styles.uploadSub}>
                Upload a CSV or Excel file with employee names, amounts, and wallet IDs. Salaries
                will be deposited directly into employee wallets.
              </Text>
              <Button
                title="Choose File"
                variant="secondary"
        ...
        <Pressable style={styles.modalBg} onPress={() => setShowUpload(false)}>
          <View style={styles.modalCard} onStartShouldSetResponder={() => true}>
            ...
            <Text style={styles.modalSub}>
              5 employees detected. Payroll will be processed after review.
            </Text>
            <Button
