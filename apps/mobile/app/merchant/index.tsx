  ...
  return (
    <Screen bg={Colors.surface.background}>
              ...
              <View key={tx.id} style={styles.txRow}>
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        tx.type === "refund" ? Colors.accent.redSoft : Colors.accent.greenSoft,
                    },
                  ]}
                >
                  <Ionicons
                    name={tx.type === "refund" ? "arrow-undo" : "arrow-down"}
                    size={16}
                    color={tx.type === "refund" ? Colors.accent.red : Colors.accent.green}
                  />
                </View>
                <View style={{ flex: 1 }}>
                ...
                <Text
                  style={[styles.txAmount, tx.type === "refund" && { color: Colors.accent.red }]}
                >
              ...
              <View key={p.id} style={styles.promoCard}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.promoMeta}>
                    {p.discountPercent}% off · {p.redemptions} used · Expires{" "}
                    {new Date(p.validUntil).toLocaleDateString()}
                  </Text>
                </View>
                <Pressable
                  onPress={() => togglePromotion(p.id)}
                  style={[styles.toggleBtn, p.active && styles.toggleActive]}
                >
                  <Text style={[styles.toggleText, p.active && { color: Colors.white }]}>
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setShowAddPromo(false)}
      >
