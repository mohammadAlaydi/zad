import type { LoginByPhoneRequest, TokenPairResponse } from "@zadpay/validation";
...
export interface UseLoginByPhoneResult {
  mutate: (input: LoginByPhoneRequest) => Promise<{ ok: true } | { ok: false; error: ClientError }>;
  isPending: boolean;
  error: ClientError | null;
  reset: () => void;
}

export function useLoginByPhone(): UseLoginByPhoneResult {
  const [isPending, setPending] = useState(false);
  const [error, setError] = useState<ClientError | null>(null);

  async function mutate(input: LoginByPhoneRequest) {
    setPending(true);
    setError(null);
    try {
      const result = await authService.loginByPhone(input);
      if (!result.ok) {
