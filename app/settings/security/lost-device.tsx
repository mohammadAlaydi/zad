        ...
        <MotiView from={{ opacity: 0, translateY: 8 }} animate={{ opacity: 1, translateY: 0 }} transition={{ duration: 320 }}>
          ...
          <View style={{ alignItems: "center", marginBottom: 24, marginTop: 12 }}>
            ...
            <Text style={{ color: Colors.ink[500], fontFamily: "Inter_400Regular", fontSize: 13, marginTop: 8, textAlign: "center", lineHeight: 20 }}>
              Lost or stolen device? We can lock{"\n"}your account immediately to protect your funds.
            </Text>
          </View>

          {/* Actions */}
          {[
            { icon: "lock-closed-outline" as const, title: "Lock account instantly", body: "Prevents all transactions and logins." },
            { icon: "phone-portrait-outline" as const, title: "Sign out all devices", body: "Remove access from every logged-in device." },
            { icon: "call-outline" as const, title: "Contact support", body: "Speak to an agent within 2 minutes." },
          ].map((a) => (
            <View key={a.title} style={{
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: Colors.ink[900], fontFamily: "Inter_600SemiBold", fontSize: 14, marginBottom: 4 }}>{a.title}</Text>
                <Text style={{ color: Colors.ink[500], fontFamily: "Inter_400Regular", fontSize: 12 }}>{a.body}</Text>
              </View>
            </View>
          ))}
