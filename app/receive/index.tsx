...
import { View, Text, Pressable, ScrollView, KeyboardAvoidingView, Platform, Dimensions } from "react-native";
  ...
  return (
    <Screen bg={Colors.white} keyboard>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: insets.bottom + 24 }} keyboardShouldPersistTaps="handled">
          <View style={{ backgroundColor: Colors.surface.background, borderRadius: 16, padding: 14, flexDirection: "row", alignItems: "center", marginBottom: 18 }}>
            ...
            {(["username", "mobile"] as const).map((key) => (
              <Pressable key={key} onPress={() => setTab(key)} style={{ flex: 1, alignItems: "center", paddingVertical: 10, borderBottomWidth: key === tab ? 2 : 0, borderBottomColor: Colors.brand.primary }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                  <Ionicons name={key === "mobile" ? "phone-portrait-outline" : "at-circle-outline"} size={16} color={key === tab ? Colors.brand.primary : Colors.ink[400]} />
          ...
          <Text style={{ textAlign: "center", color: Colors.ink[500], fontFamily: "Inter_400Regular", fontSize: 13, marginBottom: 12 }}>{t("receive.howMuchCollect")}</Text>
          <AmountStepper value={amount} onChange={setAmount} />

          <Text style={{ color: Colors.ink[700], fontFamily: "Inter_500Medium", fontSize: 13, marginTop: 22, marginBottom: 8 }}>{t("receive.receiveFrom")}</Text>
          <Input
          ...
          {!showNote ? (
            <Pressable onPress={() => setShowNote(true)} style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 4 }}>
              <Ionicons name="add-circle" size={18} color={Colors.accent.green} />
              <Text style={{ color: Colors.accent.green, fontFamily: "Inter_600SemiBold", fontSize: 13 }}>{t("receive.addNote")}</Text>
            </Pressable>
          ) : (
            <Input
          ...
          {showQr && (
            <View style={{ alignItems: "center", marginTop: 24, backgroundColor: Colors.white, borderRadius: 20, padding: 20, shadowColor: "#101225", shadowOpacity: 0.06, shadowRadius: 16, shadowOffset: { width: 0, height: 6 }, elevation: 4 }}>
              <QRCode value={qrValue} size={width * 0.45} color={Colors.brand.primary} backgroundColor={Colors.white} />
              <Text style={{ marginTop: 12, color: Colors.ink[500], fontFamily: "Inter_400Regular", fontSize: 12, textAlign: "center" }}>{t("receive.requestPayment")}</Text>
              <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
                <Pressable style={{ flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 18, paddingVertical: 10, borderRadius: 999, backgroundColor: Colors.brand.primary }}>
      ...
      <View style={{ paddingHorizontal: 18, paddingBottom: insets.bottom + 16 }}>
        <Button title={showQr ? t("common.done") : t("common.continue")} onPress={() => (showQr ? router.back() : setShowQr(true))} />
      </View>
    </Screen>
  );
}
